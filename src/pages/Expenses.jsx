...
import { Plus, Loader2, Zap, Users, Receipt, Calendar, IndianRupee, CheckCircle2, UserCheck, Edit2, Shield, UserX, Sparkles } from 'lucide-react';
...
export default function Expenses() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 2. Auto-Sync Staff Monthly Salaries for active staff (status !== 'left')
      let hasNewStaffSalary = false;
      const activeStaffWithSalary = (staffData || []).filter((s) => {
        const isLeft = s.status === 'left' || s.status === 'inactive';
        const salary = Number(s.monthlySalary) || 0;
        return !isLeft && salary > 0;
      });

      for (const staff of activeStaffWithSalary) {
        const salaryAmt = Number(staff.monthlySalary);

        // Check if salary expense already exists for this staff in selectedMonth
        const alreadyHasSalaryForMonth = expensesData.some((exp) => {
        ...
        if (!alreadyHasSalaryForMonth) {
          const newSalaryExpense = {
            category: 'Staff Salary',
            amount: salaryAmt,
            date: `${selectedMonth}-01`,
            month: selectedMonth,
            description: `Staff Salary: ${staff.name} (${staff.roleLabel || staff.role || 'Staff'}) [Monthly Auto-Schedule]`,
            expenseType: 'salary',
            staffId: staff.id,
            ...
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div>
                ...
                <p className="text-[11px] text-indigo-700 mt-0.5">
                  एक्टिव स्टाफ की सैलरी हर महीने अपने-आप इस लिस्ट में जुड़ती है। जब कोई स्टाफ छोड़ दे, तो <strong>'Mark Left'</strong> करें — सैलरी अपने-आप रुक जाएगी।
                </p>
              </div>
            </div>
            <button
        ...
        <div className="space-y-4">
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-2xl text-xs text-blue-900 leading-relaxed">
            ...
            <ul className="list-disc list-inside mt-1 space-y-0.5">
              <li>यहाँ जिस स्टाफ की मंथली सैलरी सेट होगी, वह <strong>हर महीने Expenses & Utility में अपने-आप जुड़ जाएगी</strong>।</li>
              <li>जब कोई स्टाफ नौकरी छोड़ दे, तो बस <strong>'Mark Left'</strong> दबाएं — उसकी सैलरी अगले महीनों में अपने-आप नहीं जुड़ेगी।</li>
              <li>अगर वह वापस आ जाए, तो <strong>'Reactivate'</strong> दबाएं।</li>
            </ul>
          </div>
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        >
                          {isLeft ? '🔴 Left (छोड़ दिया)' : isInactive ? '⚪ Inactive' : '🟢 Active'}
                        </span>
                      </div>
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        User ID: {st.email} {st.phone ? `• 📞 ${st.phone}` : ''}
                      </p>
                    </div>
